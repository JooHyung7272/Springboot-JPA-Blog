...
public class DummyControllerTest {
	...
	// save함수는 id를 전달하지 않으면 insert를 해주고, id를 전달하면 해당 id에 대한 데이터가 있으면 update를 해주고 id를 전달하면 해당 id 데이터가 없으면 insert를 해요
	//email, password
	@Transactional
	@PutMapping("/dummy/user/{id}")
	public User updateUser(@PathVariable int id, @RequestBody User requestUser) { // json 데이터를 요청 => java object(messageconverter의 jackson 라이브러리가 반환해서 줌)
